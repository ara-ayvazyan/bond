...
protected:
    ...
    void Begin(const T& var) const
    {
        ...
        BOOST_ASSERT(detail::OptionalDefault<T>(var));
#endif
        (void)var;
...
public:
    ...
    To(const To<T, Protocols, OtherValidator>& other)
        : _var(other._var)
    {}
...
private:
    template <typename U, typename ProtocolsT, typename ValidatorT>
    friend class To;
    ...
    {
        using base = typename schema<T>::type::base;

        bool done = Apply<Protocols>(
            To<base, Protocols, typename Validator::template rebind<base>::type>(_var),
            value);
